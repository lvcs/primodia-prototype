...
/**
 * Create tree geometry objects
 * @returns {Object} - Object containing trunk and canopy geometry
 */
 ...
 * @returns {Object} - Object containing trunk and canopy materials
 */
export const createTreeMaterials = () => {
  const trunkMaterial = new THREE.MeshLambertMaterial({ color: TREES_TRUNK_COLOR });
  const canopyMaterial = new THREE.MeshLambertMaterial({ color: TREES_CANOPY_COLOR });
  
  return { trunkMaterial, canopyMaterial };
};
 ...
 * @param {Object} geometry - Object containing trunkGeometry and canopyGeometry
 * @param {Object} materials - Object containing trunkMaterial and canopyMaterial
 * @returns {THREE.Group} - Tree group containing trunk and canopy
 */
export const createSingleTree = (x, y, z, geometry, materials) => {
  const { trunkGeometry, canopyGeometry } = geometry;
  const { trunkMaterial, canopyMaterial } = materials;
 ...
 */
export const generateTreesForTile = (tileData, geometry, materials) => {
  ...
  for (let i = 0; i < treeCount; i++) {
    ...
    if (tileData.polygonVertices && tileData.polygonVertices.length >= 3) {
      ...
      treePos = generatePointInPolygon(tileData.polygonVertices, tileData.center, sphereRadius);
    } else {
      // Fallback to circular approximation
      treePos = generatePointInCircle(tileData.center, tileData.area, sphereRadius);
    }
    
    const tree = createSingleTree(treePos.x, treePos.y, treePos.z, geometry, materials);
 ...
 */
export const generatePointInPolygon = (polygonVertices, tileCenter, sphereRadius) => {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (isPointInSphericalPolygon(testPos, polygonVertices, tileCenter)) {
      return testPos;
    }
  }
 ...
 */
export const isPointInSphericalPolygon = (point, polygonVertices, center) => {
  // Normalize all points to unit sphere for consistent calculations
  const pointNorm = normalizeVector(point);
  const centerNorm = normalizeVector(center);
  const verticesNorm = polygonVertices.map(v => normalizeVector(v));
  ...
  for (let i = 0; i < verticesNorm.length; i++) {
    ...
    // Normalize edges
    const edge1Norm = normalizeVector(edge1);
    const edge2Norm = normalizeVector(edge2);
 ...
 * @returns {Object} - {x, y, z} coordinates on sphere surface
 */
export const generatePointInCircle = (tileCenter, tileArea, sphereRadius) => {
 ...
 * @param {Object} geometry - Tree geometry objects
 * @param {Object} materials - Tree materials
 * @returns {Object} - Object containing treeGroup and individual tree groups for management
 */
export const addTreesToScene = (tiles, scene, geometry, materials) => {
  const treeGroup = new THREE.Group();
  const treeGroups = [];
  
  tiles.forEach(tile => {
    if (shouldHaveTrees(tile.terrainId)) {
      const tileTreeGroup = generateTreesForTile({
        id: tile.id,
        center: tile.center || { x: 0, y: 0, z: 0 },
        area: tile.area || 0,
        terrainId: tile.terrainId,
        polygonVertices: tile.polygonVertices
      }, geometry, materials);
      
      treeGroup.add(tileTreeGroup);
      treeGroups.push(tileTreeGroup);
    }
  });
  
  scene.add(treeGroup);
  
  return { treeGroup, treeGroups };
};

/**
 * Remove trees from the scene and dispose of resources
 * @param {THREE.Scene} scene - Three.js scene to remove trees from
 * @param {THREE.Group} treeGroup - Main tree group to remove
 * @param {Array} treeGroups - Array of individual tree groups to dispose
 */
export const clearTrees = (scene, treeGroup, treeGroups) => {
  // Remove from scene
  if (treeGroup) {
...
/**
 * Dispose of tree geometry and materials
 * @param {Object} geometry - Tree geometry objects to dispose
 * @param {Object} materials - Tree materials to dispose
 */
...
// Default export for backward compatibility - now returns an object with all functions
export default {
  createTreeGeometry,
  createTreeMaterials,
  createSingleTree,
