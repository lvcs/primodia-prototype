...
// Mock the SeedableRandom factory
vi.mock('@/game/core/random', async () => {
  const actual = await vi.importActual('@/game/core/random');
  function MockSeedableRandom(seed) {
    let callCount = 0;
    return {
      seed,
      nextFloat() {
  ...
  return { ...actual, SeedableRandom: MockSeedableRandom };
});
